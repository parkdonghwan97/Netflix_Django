...
class Movie(models.Model):
    ...
    descrition = models.TextField(blank=True,null=True) # 영화 설명
    created = models.DateTimeField(auto_now_add=True) # 날짜 추가 
    uuid = models.UUIDField(default=uuid.uuid4)
    type = models.CharField(max_length=10,choices=MOVIE_CHOICES ) # 영화 타입, 시즌영화인지 단일 영화인지 
    videos = models.ManyToManyField('Video') # 비디오 관련 
    flyer = models.ImageField(upload_to='flyers')# 포스터 or 전단지(?) 이미지
    age_limit = models.CharField(max_length=10,choices=AGE_CHOICES) 
